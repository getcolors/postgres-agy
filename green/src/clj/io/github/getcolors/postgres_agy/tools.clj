(ns io.github.getcolors.postgres-agy.tools
...
(def infrastructure-tool "postgres-agy-infrastructure")
(def dns-tool "postgres-agy-dns")
(def ansible-local-tool "postgres-agy-ansible-local")
(def cluster-tool "postgres-agy-cluster")
(def acceptance-tool "postgres-agy-acceptance")
(def tofu-tools [infrastructure-tool dns-tool])

(def ^:private root "io.github.getcolors.postgres-agy.tools")
(def ^:private template-opts sc/preserve-jinja-delimiters)

(defn tool-dir [opts tool]
  (green-cli/stage-dir opts tool {:default-profile "postgres-agy"}))
...
(defn output-params
  ...
  [result]
  (cluster/output-params {:tofu/outputs (:postgres-agy/outputs result)}))
  ...
  (resolve-infrastructure
   ...
   (tofu/tofu-with-spec opts (infrastructure-specs opts)
                        {:dir (tool-dir opts infrastructure-tool)
                         ...
                         :output-key :postgres-agy/outputs})))
...
(defn load-infrastructure-step
  ...
  `:postgres-agy/state`, or a fresh read when nothing was. An unreadable
  ...
  (let [event (:green/event opts)
        state (or (:postgres-agy/state opts) (cluster/read-state opts state-output))
        adopted (cluster/adopt-state validate/spec (dissoc opts :postgres-agy/state) event state)
    ...
    (if (wf/failed? adopted)
      ...
      (let [checked (checked adopted)]
        (if (wf/failed? checked)
          checked
          (assoc checked :postgres-agy/infrastructure-present? present?))))))
...
(defn dns-step
  ...
  (tofu/tofu-with-spec opts (dns-specs opts)
                       {:dir (tool-dir opts dns-tool)
                        ...
                        :output-key :postgres-agy/dns-outputs}))
...
(defn cluster-step
  ...
  (if (and (= :delete (:green/event opts))
           (false? (:postgres-agy/infrastructure-present? opts)))
    ...
    (ansible/ansible-with-spec
     ...
     {:dir (tool-dir opts cluster-tool)
      ...
      :recap-key :postgres-agy/cluster-recap}
     (cluster-specs opts))))
