(ns io.github.getcolors.postgres-agy.tools
  ...
  (:require [cheshire.core :as json]
            ...
            [io.github.getcolors.postgres-agy.compute :as compute]
...
(defn load-infrastructure-step [opts]
  (if (or (= :build (:green/event opts)) (:green/dry-run opts)) (infrastructure-step opts)
      (let [result (inspection/read-deployment opts)]
        (case (:status result)
          "destroyed" (assoc opts :postgres-agy/already-destroyed true :green/exit 0)
          "present" (cond-> (assoc opts :colors-compute/cluster (:cluster result) :colors-compute/shared (:shared result) :postgres-agy/infrastructure-present? true :green/exit 0)
