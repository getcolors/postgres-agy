(ns io.github.getcolors.postgres-agy.ssh-config-test
  (:require [clojure.string :as str]
            [clojure.test :refer [deftest is testing]]
            [io.github.getcolors.postgres-agy.ssh-config :as ssh-config]))

(def opts {:profile "postgres-agy-digitalocean" :cluster-nodes 3})

(deftest the-deployment-claims-one-alias-per-node-and-the-bare-profile
  ;; `ssh postgres-agy-digitalocean` is what the standard promises; the numbered
  ;; aliases are what make a cluster operable, since half of running one is
  ;; reaching a specific node.
  (is (= ["postgres-agy-digitalocean" "postgres-agy-digitalocean-0"
          "postgres-agy-digitalocean-1" "postgres-agy-digitalocean-2"]
         (ssh-config/aliases opts))))

(deftest the-identity-file-stays-unexpanded
  (is (= "~/.ssh/postgres-agy-digitalocean" (ssh-config/identity-file opts))))
...
(deftest a-foreign-stanza-is-found-for-any-alias-not-just-the-first
  (let [lines (str/split-lines "Host something\n  HostName 1.2.3.4\n\nHost postgres-agy-digitalocean-2\n  HostName 5.6.7.8\n")]
    (is (nil? (ssh-config/foreign-stanza-line lines "postgres-agy-digitalocean")))
    (is (= 4 (ssh-config/foreign-stanza-line lines "postgres-agy-digitalocean-2")))))
...
(deftest our-own-managed-block-is-not-foreign-for-any-alias-in-it
  ...
  ;; `# BEGIN postgres-agy-digitalocean-0 …`, never find it, and refuse to
  ;; converge because of a block this package wrote itself.
  (let [lines (str/split-lines
               (str "# BEGIN postgres-agy-digitalocean ANSIBLE MANAGED BLOCK\n"
                    "Host postgres-agy-digitalocean\n  HostName 1.2.3.4\n"
                    "Host postgres-agy-digitalocean-0\n  HostName 1.2.3.4\n"
                    "Host postgres-agy-digitalocean-1\n  HostName 1.2.3.5\n"
                    "Host postgres-agy-digitalocean-2\n  HostName 1.2.3.6\n"
                    "# END postgres-agy-digitalocean ANSIBLE MANAGED BLOCK\n"))
        marker "postgres-agy-digitalocean"]
...
(deftest a-node-stanza-outside-our-block-is-still-foreign
  (let [lines (str/split-lines
               (str "# BEGIN postgres-agy-digitalocean ANSIBLE MANAGED BLOCK\n"
                    ...
                    "# END postgres-agy-digitalocean ANSIBLE MANAGED BLOCK\n"
                    "Host postgres-agy-digitalocean-1\n  HostName 9.9.9.9\n"))]
    (is (= 5 (ssh-config/foreign-stanza-line lines "postgres-agy-digitalocean-1" "postgres-agy-digitalocean")))))
