...
;; anywhere else it would stamp an unrelated SHA.
(defn git [& args] (let [{:keys [exit out]} (apply sh/sh "git" args)] (when (zero? exit) (str/trim out))))

(defn stamp-green [s sha]
  (when (re-find #"\(def \^:private postgres-agy-sha (?:nil|\"[0-9a-f]{40}\")\)" s)
    (str/replace-first s #"\(def \^:private postgres-agy-sha (?:nil|\"[0-9a-f]{40}\")\)"
                       (str "(def ^:private postgres-agy-sha \"" sha "\")"))))

(defn stamp-red [s sha]
  (let [pinned (str "\"package-postgres-agy-red\": \"github:getcolors/postgres-agy#" sha "\",")]
    (cond (str/includes? s "\"package-postgres-agy-red\": null,")
          (str/replace-first s "\"package-postgres-agy-red\": null," pinned)
          (re-find #"\"package-postgres-agy-red\": \"github:getcolors/postgres-agy#[0-9a-f]{40}\"," s)
          (str/replace-first s #"\"package-postgres-agy-red\": \"github:getcolors/postgres-agy#[0-9a-f]{40}\"," pinned))))

(def blue-unpinned-meta "# dependencies = []\n# ///")
(defn blue-pinned-meta [sha]
  (str "# dependencies = [\"package-postgres-agy-blue\", \"blue\"]\n"
       "#\n"
       "# [tool.uv.sources]\n"
       "# package-postgres-agy-blue = { git = \"https://github.com/getcolors/postgres-agy.git\", rev = \"" sha "\", subdirectory = \"blue\" }\n"
...
(defn stamp-blue [s sha]
  ...
  (cond (str/includes? s blue-unpinned-meta)
        (-> s
            ...
            (str/replace-first #"(?s)# UNPINNED:.*?POSTGRES_AGY_LIB_ROOT=/path/to/postgres-agy\n"
                               "# Stamped by `bb pin`. POSTGRES_AGY_LIB_ROOT=/path/to/postgres-agy still overrides the\n# pin with a working tree.\n"))
        (re-find #"postgres-agy\.git\", rev = \"[0-9a-f]{40}\"" s)
        (str/replace-first s #"postgres-agy\.git\", rev = \"[0-9a-f]{40}\""
                           (str "postgres-agy.git\", rev = \"" sha "\""))))

(def sites
  [{:path "../skills/package-postgres-agy-green/green" :stamp stamp-green}
   {:path "../skills/package-postgres-agy-red/red" :stamp stamp-red}
   {:path "../skills/package-postgres-agy-blue/blue" :stamp stamp-blue}])

(let [dirty (git "status" "--porcelain") sha (git "rev-parse" "HEAD") remotes (git "branch" "-r" "--contains" sha)]
  (cond (seq dirty) (do (binding [*out* *err*] (println "postgres-agy working tree is dirty; commit before pinning")) (System/exit 2))
        (not (str/includes? (str remotes) "origin/")) (do (binding [*out* *err*] (println "postgres-agy HEAD is not pushed")) (System/exit 2))
