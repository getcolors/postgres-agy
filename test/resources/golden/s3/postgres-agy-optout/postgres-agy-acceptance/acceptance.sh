#!/usr/bin/env bash
# Post-convergence health checks for postgres-agy.
set -uo pipefail

HOST="postgres-agy.bigconfig.online"
PRIMARY_PORT=5432
REPLICA_PORT=5433
DB="appdb"
USER="postgres"
...
ALIASES=("postgres-agy-optout-0" "postgres-agy-optout-1" "postgres-agy-optout-2" )
...
stamp=$(on_node "cat /var/lib/postgresql/.postgres-agy-restore-check 2>/dev/null || true")
if [ -n "$stamp" ]; then
