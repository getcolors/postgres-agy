...
async def start_step(original: dict, env: dict | None = None, reader=None) -> dict:
    ...
    async def after(opts, _env, ctx):
        handed = {**opts, "postgres-agy/state": state} if _real_lifecycle_event(ctx) else opts
...
def wire_fn(step: str, run_opts: dict):
    if run_opts.get("blue/event") == "delete":
        return {
            ...
            "postgres-agy/infrastructure": (tools.infrastructure_step, "postgres-agy/ssh-cleanup"),
            "postgres-agy/ssh-cleanup": (ssh.cleanup_step, "postgres-agy/generated-cleanup"),
