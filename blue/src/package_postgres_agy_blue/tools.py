...
async def load_infrastructure_step(opts):
    ...
    if result['status'] == 'destroyed':
        return {**opts, 'postgres-agy/already-destroyed': True, 'blue/exit': 0}
    if result['status'] != 'present':
        return _refuse(opts, ['compute state unavailable; legacy monolithic state requires explicit migration'])
    handed = {**opts, 'colors-compute/cluster': result['cluster'], 'colors-compute/shared': result.get('shared', {}), 'postgres-agy/infrastructure-present?': True, 'blue/exit': 0}
    path = result.get('key', {}).get('private_key_path')
    if path:
