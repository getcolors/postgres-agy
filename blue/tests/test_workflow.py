...
from package_postgres_agy_blue import tools, validate, workflow
from test_tools import recorded


def test_lifecycle_order_preserves_application_cleanup_before_compute():
    assert workflow.wire_fn('postgres-agy/start', {'blue/event': 'create'})[1] == 'postgres-agy/infrastructure'
    assert workflow.wire_fn('postgres-agy/dns', {'blue/event': 'delete'})[1] == 'postgres-agy/infrastructure'
    assert workflow.wire_fn('postgres-agy/infrastructure', {'blue/event': 'delete'})[1] == 'postgres-agy/generated-cleanup'


async def test_native_build_renders_shared_nodes_and_every_application_stage(tmp_path):
    result = await run(workflow.postgres_agy_workflow, fixture({'blue/event': 'build', 'workdir': str(tmp_path)}))
...
async def test_delete_read_refuses_legacy_and_unreadable_state(monkeypatch):
    ...
    assert (await tools.load_infrastructure_step(fixture({'blue/event': 'delete'})))['postgres-agy/already-destroyed'] is True
...
async def test_native_optout_build_keeps_external_identity_reference(tmp_path):
    result = await run(workflow.postgres_agy_workflow, optout({'blue/event': 'build', 'workdir': str(tmp_path)}))
    assert result['blue/exit'] == 0, result.get('blue/err')
