...
from package_postgres_agy_blue import ssh_config

opts = fixture({"profile": "postgres-agy-digitalocean"})
...
def test_the_deployment_claims_one_alias_per_node_and_the_bare_profile():
    ...
    assert ssh_config.aliases(opts) == [
        "postgres-agy-digitalocean", "postgres-agy-digitalocean-0",
        "postgres-agy-digitalocean-1", "postgres-agy-digitalocean-2"]


def test_the_identity_file_stays_unexpanded():
    assert ssh_config.identity_file(opts) == "~/.ssh/postgres-agy-digitalocean"


def test_a_foreign_stanza_is_found_for_any_alias_not_just_the_first():
    lines = ("Host something\n  HostName 1.2.3.4\n\n"
             "Host postgres-agy-digitalocean-2\n  HostName 5.6.7.8\n").splitlines()
    assert ssh_config.foreign_stanza_line(lines, "postgres-agy-digitalocean") is None
    assert ssh_config.foreign_stanza_line(lines, "postgres-agy-digitalocean-2") == 4
...
def test_our_own_managed_block_is_not_foreign_for_any_alias_in_it():
    ...
    lines = ("# BEGIN postgres-agy-digitalocean ANSIBLE MANAGED BLOCK\n"
    ...
    for alias in ssh_config.aliases(opts):
        assert ssh_config.foreign_stanza_line(lines, alias, "postgres-agy-digitalocean") is None, alias
...
def test_a_node_stanza_outside_our_block_is_still_foreign():
    lines = ("# BEGIN postgres-agy-digitalocean ANSIBLE MANAGED BLOCK\n"
             ...
             "# END postgres-agy-digitalocean ANSIBLE MANAGED BLOCK\n"
             "Host postgres-agy-digitalocean-1\n  HostName 9.9.9.9\n").splitlines()
    assert ssh_config.foreign_stanza_line(lines, "postgres-agy-digitalocean-1", "postgres-agy-digitalocean") == 5
...
def test_the_refusal_is_reported_as_a_failed_step(monkeypatch, tmp_path):
    ...
    config.write_text("Host postgres-agy-digitalocean-1\n  HostName 9.9.9.9\n")
    monkeypatch.setenv("HOME", str(tmp_path))
    refused = ssh_config.preflight(opts)
    assert refused["blue/exit"] == 1
    assert "postgres-agy-digitalocean-1" in refused["blue/err"]
