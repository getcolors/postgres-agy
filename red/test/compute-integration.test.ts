...
import {postgresAgyWorkflow} from '../src/workflow.ts';
...
test('native SDK build renders one shared stack plus three nodes without SSH files',async()=>{
 const dir=mkdtempSync(join(tmpdir(),'postgres-agy-build-'));try{
  const opts={...fixture(),workdir:dir,'red/event':'build'};
  const result=await run(postgresAgyWorkflow,opts);expect(result['red/exit']).toBe(0);
...
test('inventory readers refuse unknown or partial state and propagate destruction',async()=>{
 ...
 expect((await tools.loadInfrastructureStep(opts,async()=>({status:'destroyed'}) as any))['postgres-agy/already-destroyed']).toBe(true);
