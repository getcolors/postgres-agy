...
// mode when desired state carries no `digitalocean-ssh-keys`, an ed25519 key
// named after the profile in `~/.ssh`, the create matrix, the DigitalOcean
// REST preflight, and a cleanup that runs only after a successful destroy.
...
// from `$HOME` and does not commit rendered output; this package does commit
// goldens, so on `build` the rendered paths must not name the operator's home
...
export function withMachineKey(opts: Opts): Opts {
  ...
  const profile = String(opts.profile ?? "postgres-agy");
