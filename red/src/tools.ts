...
export const infrastructureTool = "postgres-agy-infrastructure";
export const dnsTool = "postgres-agy-dns";
export const ansibleLocalTool = "postgres-agy-ansible-local";
export const clusterTool = "postgres-agy-cluster";
export const acceptanceTool = "postgres-agy-acceptance";
...
export function toolDir(opts: Opts, tool: string): string {
  return stageDir(opts, tool, { defaultProfile: "postgres-agy" });
}
...
export async function loadInfrastructureStep(opts:Opts,reader:typeof read_deployment=read_deployment):Promise<Opts>{
 ...
 const result:any=await reader(opts);if(result.status==='destroyed')return {...opts,'postgres-agy/already-destroyed':true,'red/exit':0};
 ...
 return {...opts,'colors-compute/cluster':result.cluster,'colors-compute/shared':result.shared??{},'postgres-agy/infrastructure-present?':true,...(result.key?.private_key_path?{'ssh-private-key-path':result.key.private_key_path}:{}),'red/exit':0};
}
...
export async function dnsStep(opts: Opts): Promise<Opts> {
  return tofu.tofuWithSpec(opts, dnsSpecs(opts), {
    ...
    outputKey: "postgres-agy/dns-outputs",
  });
}
...
export async function clusterStep(opts: Opts): Promise<Opts> {
  if (opts["red/event"] === "delete" &&
      opts["postgres-agy/infrastructure-present?"] === false) {
  ...
  return ansible.ansibleWithSpec(opts, {
    ...
    recapKey: "postgres-agy/cluster-recap",
  }, clusterSpecs(opts));
}
