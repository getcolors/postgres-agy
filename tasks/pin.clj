...
(def ^:private site
  "Where a copied payload records which postgres-agy commit to resolve."
  {:path "skills/package-postgres-agy-green/green"
   :rx #"\(def \^:private postgres-agy-sha (nil|\"[0-9a-f]{40}\")\)"})
  ...
  (let [file (io/file (:path site))]
    (cond
      ...
      (let [text (slurp file)
        ...
        (if-not current
          {:green/exit 2
           :green/err (str "could not locate postgres-agy-sha in " (:path site))}
          (let [[head err] (repo-head "." "postgres-agy")]
            (cond
